//! `devai-memory` — the memory engine: remember (dedup/revise) + recall (blend).
//!
...
use devai_chunk::{content_hash, memory_chunks, MemoryChunkConfig};
use devai_core::types::{SearchFilter, VectorMetadata, VectorPoint};
use devai_embed::EmbeddingProvider;
use devai_store::{Memory, MemoryStats, Store};
...
mod tests {
    use super::*;
    use devai_embed::Result as EmbedResult;
