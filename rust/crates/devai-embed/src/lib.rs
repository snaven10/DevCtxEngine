//! `devai-embed` — embedding providers for DevAI.
//!
...
impl EmbedSettings {
    ...
    pub fn from_config(cfg: &devai_core::config::Embeddings) -> Self {
        use devai_core::config::Offline;
        ...
        Self {
            ...
            offline,
            api_key: None,
            endpoint: std::env::var("DEVAI_EMBED_ENDPOINT").ok(),
            custom_dimension: std::env::var("DEVAI_EMBED_DIMENSION")
                .ok()
                .and_then(|s| s.parse().ok()),
            model_dir: std::env::var("DEVAI_MODEL_DIR").ok().map(PathBuf::from),
        }
    }
}
