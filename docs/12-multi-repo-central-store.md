...
( cd "$(git rev-parse --show-toplevel)" \
  && DEVAI_STATE_DIR="/abs/path/to/central/state" \
     ...
     "/abs/path/to/devai" index --incremental ) >/dev/null 2>&1 &
# <<< DEVAI_AUTO_INDEX <<<
```
...
`*_worktree`, `*/worktrees/*`, etc.). The `exit 0` lets the rest of the hook file continue
if there are other post-commit tasks outside the devai block; to abort the entire hook
instead, use `exit 0` before the devai block, not inside it.
...
`ml-granite` on CPU). Set a safe ceiling in the hook env and in your MCP `env` block:

```bash
DEVAI_EMBED_MAX_CHARS=2048   # default: 4096
```

`devai hooks install` now embeds this value automatically when `DEVAI_EMBED_MAX_CHARS` is
