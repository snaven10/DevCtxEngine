...
mod tests {
    ...
    fn apply_inserts_and_preserves_other_keys() {
        ...
        let entry = json!({ "command": "devctx", "args": ["mcp"] });
        let out = apply_config(Some(existing), "devctx", entry, false);
        assert_eq!(out["theme"], "dark");
        assert_eq!(out["mcpServers"]["other"]["command"], "x");
        assert_eq!(out["mcpServers"]["devctx"]["command"], "devctx");
    }

    #[test]
    fn apply_creates_structure_from_nothing() {
        let out = apply_config(None, "devctx", json!({ "command": "d" }), false);
        assert_eq!(out["mcpServers"]["devctx"]["command"], "d");
    }

    #[test]
    fn apply_removes_entry() {
        let existing = json!({ "mcpServers": { "devctx": { "command": "d" }, "keep": {} } });
        let out = apply_config(Some(existing), "devctx", json!({}), true);
        assert!(out["mcpServers"].get("devctx").is_none());
    ...
    fn entry_has_project_arg_and_env() {
        let e = build_entry(
            Path::new("/usr/bin/devctx"),
            Path::new("/home/u/proj"),
            &["KEY=VAL".to_string()],
        )
        .unwrap();
        assert_eq!(e["command"], "/usr/bin/devctx");
